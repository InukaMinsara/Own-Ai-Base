...
def parse_instruction_file(path):
    ...
    for chunk in text.split("<BOS>"):
        ...
        user_match = re.search(r"User:s*(.*?)s*Assistant:", chunk, re.S)
        if not user_match:
            continue
    ...
    if not unique:
        raise ValueError(f"No valid instruction examples found in {path}")
...
Assistant:"
    ...
    tokens = [bos_id] + prefix_ids + answer_ids + [eos_id]
    loss_mask = [0] * (1 + len(prefix_ids)) + [1] * (len(answer_ids) + 1)
...
def split_examples(examples):
    ...
    split = max(1, int(len(examples) * TRAIN_RATIO))
...
def encoded_examples(examples, tokenizer):
    ...
    for user_text, answer_text in examples:
        item = make_example_ids(tokenizer, user_text, answer_text)
        if item is not None:
            usable.append(item)
...
def collate(batch, pad_id):
    max_len = min(BLOCK_SIZE + 1, max(len(x[0]) for x in batch))
    ...
    for tokens, mask in batch:
        ...
        if len(tokens) < max_len:
            ...
            tokens = torch.cat(
                [tokens, torch.full((pad,), pad_id, dtype=torch.long)]
            )
            mask = torch.cat(
                [mask, torch.zeros(pad, dtype=torch.float32)]
            )
...
def make_batch_stream(encoded, batch_size, pad_id, device):
    while True:
        ...
        for start in range(0, len(order), batch_size):
            ...
            batch = [encoded[i] for i in indices]
            x, y, mask = collate(batch, pad_id)
            yield x.to(device), y.to(device), mask.to(device)
...
@torch.no_grad()
def evaluate(model, encoded, pad_id, device):
    model.eval()
    losses = []
    ...
    for start in range(0, len(encoded), BATCH_SIZE):
        ...
        losses.append(masked_loss(model, x, y, mask).item())
...
def main():
    ...
    for required in (BASE_CHECKPOINT, BASE_TOKENIZER, INSTRUCTION_FILE):
        if not required.exists():
            raise FileNotFoundError(f"Missing required file: {required}")

    tokenizer = OwnTokenizer(vocab_size=512)
    tokenizer.load(BASE_TOKENIZER)

    examples = parse_instruction_file(INSTRUCTION_FILE)
    train_examples, val_examples = split_examples(examples)

    train_encoded = encoded_examples(train_examples, tokenizer)
    val_encoded = encoded_examples(val_examples, tokenizer)

    print("Instruction pairs:", len(examples))
    print("Train pairs:", len(train_encoded))
    print("Validation pairs:", len(val_encoded))
    ...
    print("Starting from:", BASE_CHECKPOINT)
    print("Loss target: assistant response only")
    print("-" * 56)
    ...
    stream = make_batch_stream(train_encoded, BATCH_SIZE, pad_id, device)
    ...
    for step in range(MAX_STEPS):
        ...
        if step % EVAL_INTERVAL == 0 or step == MAX_STEPS - 1:
            ...
            val_loss = evaluate(model, val_encoded, pad_id, device)
                ...
                torch.save(
                    {
                        ...
                        "instruction_pairs": len(examples),
                        "masking": "assistant_response_only",
                    },
                    OUT_CHECKPOINT,
                )

                tokenizer.save(OUT_TOKENIZER)
                print("  Saved new v6 SFT checkpoint.")
            else:
                bad_evals += 1
                print(f"  No improvement ({bad_evals}/{PATIENCE})")
    ...
    print("Model:", OUT_CHECKPOINT)
    print("Tokenizer:", OUT_TOKENIZER)
    print("=" * 56)
